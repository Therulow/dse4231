...
##########################################
# Hybrid SC for recreational marijuna

# Main results folder
if (!dir.exists("results_rec_hybrid")) {
  dir.create("results_rec_hybrid", recursive = TRUE)
}

# Plots folder
if (!dir.exists("results_rec_hybrid/plots")) {
  dir.create("results_rec_hybrid/plots", recursive = TRUE)
}

# check block structure
png("results_rec_hybrid/plots/panelview_rec.png", width = 1200, height = 800)
...
failed_models   <- list()
chosen_nu <- list() # to store chosen nu
...
for (outcome_var in crime_outcomes) {
  ...
  fit <- tryCatch(
    multisynth(
      ...
      unit = STATE,
      time = YEAR,
      data = df_trim_rec # heuristic choice of nu
    ),
    error = function(e) e
  )
  ...
  # store chosen nu
  chosen_nu[[outcome_var]] <- fit$nu
  cat("Chosen nu:", fit$nu, "\n")
  ...
  writeLines(
    summary_text,
    paste0("results_rec_hybrid/", outcome_var, "_summary.txt")
  )
  ...
  if (!is.null(att_tbl)) {
    ...
    write.csv(
      att_tbl,
      paste0("results_rec_hybrid/", outcome_var, "_att.csv"),
      row.names = FALSE
    )
  }
  ...
  # Save plot
  # -----------------------------
  plot_file <- file.path("results_rec_hybrid", "plots", paste0(outcome_var, "_multisynth_avg.png"))
...
# -----------------------------
# Save chosen nu values
# -----------------------------
if (length(chosen_nu) > 0) {
  nu_df <- data.frame(
    Outcome = names(chosen_nu),
    Nu = unlist(chosen_nu),
    row.names = NULL
  )
  ...
  write.csv(
    nu_df,
    "results_rec_hybrid/chosen_nu.csv",
    row.names = FALSE
  )
}
...
if (length(all_att_results) > 0) {
  ...
  write.csv(
    combined_att,
    "results_rec_hybrid/all_att_results.csv",
    row.names = FALSE
  )
}
...
if (length(failed_models) > 0) {
  ...
  write.csv(
    failed_df,
    "results_rec_hybrid/failed_models.csv",
    row.names = FALSE
  )
}
...
##########################################
# Hybrid SC for medical marijuna

# Main results folder
if (!dir.exists("results_med_hybrid")) {
  dir.create("results_med_hybrid", recursive = TRUE)
}

# Plots folder
if (!dir.exists("results_med_hybrid/plots")) {
  dir.create("results_med_hybrid/plots", recursive = TRUE)
}

# check block structure
png("results_med_hybrid/plots/panelview_med.png", width = 1200, height = 800)
...
for (outcome_var in crime_outcomes) {
  ...
  writeLines(
    summary_text,
    paste0("results_med_hybrid/", outcome_var, "_summary.txt")
  )
  ...
  if (!is.null(att_tbl)) {
    ...
    write.csv(
      att_tbl,
      paste0("results_med_hybrid/", outcome_var, "_att.csv"),
      row.names = FALSE
    )
  }
  ...
  # Save plot
  # -----------------------------
  plot_file <- file.path("results_med_hybrid", "plots", paste0(outcome_var, "_multisynth_avg.png"))
  dir.create(dirname(plot_file), recursive = TRUE, showWarnings = FALSE)
...
if (length(chosen_nu) > 0) {
  ...
  write.csv(
    nu_df,
    "results_med_hybrid/chosen_nu.csv",
    row.names = FALSE
  )
}
...
if (length(all_att_results) > 0) {
  ...
  write.csv(
    combined_att,
    "results_med_hybrid/all_att_results.csv",
    row.names = FALSE
  )
}
...
if (length(failed_models) > 0) {
  ...
  write.csv(
    failed_df,
    "results_med_hybrid/failed_models.csv",
    row.names = FALSE
  )
}
